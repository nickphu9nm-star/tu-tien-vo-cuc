import { Item, ItemType, ItemRarity, SecretRealm, EquipSlot, ItemStats, LeaderboardEntry, MarketListing, WorldBoss, StatLine, ChatMessage, Player } from './types';
...
export const SECRET_REALMS: SecretRealm[] = [
  ...
  { id: 2, name: "Rừng Mê Vụ", minRealmIdx: 1, description: "Sương mù dày đặc (Rơi: Phàm).", dropRate: 10 },
  { id: 3, name: "Hang Động U Tối", minRealmIdx: 2, description: "Nơi dơi quỷ trú ngụ (Rơi: Phàm).", dropRate: 25 },
  { id: 4, name: "Thác Nước Bạc", minRealmIdx: 3, description: "Thủy quái canh giữ (Rơi: Linh/Phàm).", dropRate: 25 },
  { id: 5, name: "Đỉnh Núi Lửa", minRealmIdx: 4, description: "Hỏa tinh nóng bỏng (Rơi: Huyền/Linh).", dropRate: 25 },
  { id: 6, name: "Cổ Mộ Hoang Phế", minRealmIdx: 5, description: "Mộ phần cổ đại (Rơi: Địa/Huyền).", dropRate: 25 },
  { id: 7, name: "Vực Thẳm Băng Giá", minRealmIdx: 6, description: "Lạnh thấu xương (Rơi: Thiên/Địa).", dropRate: 25 },
  { id: 8, name: "Thiên Đảo", minRealmIdx: 7, description: "Đảo bay trên trời (Rơi: Tiên/Thiên).", dropRate: 25 },
  { id: 9, name: "Hư Không Chi Môn", minRealmIdx: 8, description: "Cổng hư không (Rơi: Thần/Tiên).", dropRate: 15 },
];
...
const EQUIPMENT_TEMPLATES = [
  ...
  { slot: EquipSlot.FEET, baseName: 'Hài', mainStat: 'speed', baseValue: 0.1, desc: 'Giúp di chuyển nhanh.' },
...
export const generateEquipment = (rarity: ItemRarity, forcedSlot?: EquipSlot): Item => {
  ...
  if (mainCode === 'breakthroughChance') mainVal = config.breakthroughVal;
  else if (mainCode === 'speed') mainVal = parseFloat((template.baseValue * config.multiplier).toFixed(2));
  else mainVal = Math.floor(template.baseValue * config.multiplier);
...
const BASE_ITEMS: Item[] = [
  { id: 'spirit_stone', name: 'Linh Thạch', type: ItemType.RESOURCE, rarity: ItemRarity.GREY, description: 'Tiền tệ giao dịch.', value: 1 },
  { id: 'black_iron', name: 'Hắc Thiết', type: ItemType.RESOURCE, rarity: ItemRarity.GREY, description: 'Nguyên liệu cường hóa.', value: 10 },
];
export const POSSIBLE_ITEMS: Item[] = [...BASE_ITEMS];
...
export const MOCK_LEADERBOARD: LeaderboardEntry[] = Array.from({length: 10}).map((_, i) => {
  ...
  return { rank: i + 1, name: i===0 ? 'Độc Cô Cầu Bại' : `Top ${i+1} Đại Thần`, realm: realmName, power: exactPower, equipment: equipment, stats: exactStats };
});
